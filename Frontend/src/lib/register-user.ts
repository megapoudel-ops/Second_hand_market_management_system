...
export const registerUser = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    try {
      // Call the SECURITY DEFINER SQL function that inserts directly into
      // auth.users with ALL required GoTrue fields properly set.
      // This completely bypasses Supabase's HTTP signup endpoint (and its
      // "Confirm email" setting / rate limits).
      const { data: userId, error } = await db.rpc("create_user_account", {
        p_email:     data.email,
      ...
      if (error) {
        console.error("create_user_account error:", error);
        const msg = (error.message ?? "").toLowerCase();
        // duplicate = user already exists, fine — just resend OTP
